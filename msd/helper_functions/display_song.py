...
if __name__ == '__main__':
    ...
    if not os.path.isfile(hdf5path):
        print(f'ERROR: file {hdf5path} does not exist.')
        sys.exit(0)
    h5 = hdf5_getters.open_h5_file_read(hdf5path)
    numSongs = hdf5_getters.get_num_songs(h5)
    if songidx >= numSongs:
        print(f'ERROR: file contains only {numSongs}')
        h5.close()
        sys.exit(0)

    # get all getters
    getters = list(filter(lambda x: x[:4] == 'get_', list(hdf5_getters.__dict__.keys())))
    if "get_num_songs" in getters:  # special case
    ...
    if onegetter == 'num_songs' or onegetter == 'get_num_songs':
        ...
        if onegetter not in getters:
            print(f'ERROR: getter requested: {onegetter} does not exist.')
            h5.close()
            sys.exit(0)
        getters = [onegetter]
